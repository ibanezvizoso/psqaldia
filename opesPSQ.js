...
/**
 * Carga de datos (Exactamente igual que Anestesia)
 */
async function iniciarExamen(año) {
    ...
    const url = `${window.WORKER_URL}?sheet=${pestaña}`; // URL idéntica a Anestesia
    ...
    try {
        ...
        modalData.innerHTML = `
            <div style="padding:2rem; text-align:center; color:var(--text-main);">
                <i class="fas fa-wifi fa-2x" style="color:#ef4444; margin-bottom:1rem;"></i><br>
                No se pudo cargar el examen.<br>
                <small style="color:var(--text-muted);">${error.message}</small>
                <br><br>
                <button onclick="openExamenSelector()" class="btn">Reintentar</button>
            </div>`;
    }
}
            ...
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:2rem; border-bottom: 2px solid var(--border); padding-bottom:1.5rem;">
                <div>
                    ...
                    <p style="margin:5px 0 0; font-size:0.75rem; color:var(--text-muted);">
                        Cargadas <span id="cont-preg">${preguntasVisibles}</span> de ${preguntasExamen.length} preguntas
                    </p>
                </div>
                ...
                <div style="display:flex; flex-direction:column; gap:10px;">
                    ${['A', 'B', 'C', 'D'].map((letra, idx) => `
                        <label style="display:flex; align-items:center; gap:12px; padding:15px; background:var(--card); border:1px solid var(--border); border-radius:12px; cursor:pointer; color:var(--text-main);">
                            <input type="radio" name="preg-${realIndex}" value="${letra}" style="accent-color:var(--primary); width:18px; height:18px;" onclick="respuestasUsuario[${realIndex}] = '${letra}'">
                            <span style="font-size:0.95rem;">${p.opciones[idx]}</span>
                        </label>
                    `).join('')}
                </div>
                <button onclick="revelarIndividual(${realIndex})" style="margin-top:1.5rem; background:none; border:none; color:var(--text-muted); font-weight:800; font-size:0.7rem; text-transform:uppercase; cursor:pointer; display:flex; align-items:center; gap:5px;">
                    <i class="fas fa-lightbulb"></i> Ver explicación
                </button>
                <div id="feedback-${realIndex}" style="display:none; margin-top:1.2rem; padding:1.2rem; background:var(--card); border-left:4px solid var(--primary); border-radius:12px; font-size:0.9rem; color:var(--text-main);">
