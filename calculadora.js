...
const CONFIG = {
    FILA_INICIO_FARMACOS: 0,      // Haloperidol en índice 0
    COL_INICIO_DESTINOS: 6        // Destinos empiezan en Columna G (6)
};

window.iniciarInterfazCalculadora = async function() {
    const container = document.getElementById('modalData');

    // 1. ESTILOS COMPLETOS (Timeline + Alertas + Diseño Pulido)
...
// 3. TRADUCTOR CON LIMITADOR DE DOSIS MÁXIMA
window.traducirPasos = function(raw, dosisActual, dosisObjetivo) {
    ...
    bloques.forEach(bloque => {
        ...
        if (sujeto === 'ACTUAL') {
            ...
            // Lógica de tope: No superar dosisObjetivo
            let dosisPaso = 0;
            const numExtraido = parseFloat(valor.replace(/[^0-9.]/g, ''));

            if (valor === 'TARGET' || accion === 'TITULAR_PROGRESIVO') {
                dosisPaso = dosisObjetivo;
            } else if (valor.includes('%')) {
                dosisPaso = dosisObjetivo * numExtraido / 100;
            } else {
                dosisPaso = numExtraido;
            }
            ...
            if (dosisPaso >= dosisObjetivo) {
                ...
                desc = `Iniciar/Subir nuevo a <b>${dosisPaso.toFixed(1)} mg</b>.`;
            }
        }
...
window.ejecutarCalculo = function() {
    ...
    let rawInstr = (window.dbRaw[fila] && window.dbRaw[fila][col]) ? window.dbRaw[fila][col].toString() : "";
