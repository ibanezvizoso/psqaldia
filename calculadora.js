 ...
 * LÓGICA FINAL:
 * 1. Selector Origen: Columna A (desde A2)
 * 2. Selector Destino: Columna A (desde A2)
 * 3. Matriz Switch: Fila (Origen en A) / Columna (Destino en Fila 13)
 */
...
window.iniciarInterfazCalculadora = async function() {
    ...
    if (!document.getElementById('calc-internal-styles')) {
        ...
        styleTag.innerHTML = `
            ...
            .btn-ejecutar { margin-top: 1rem; padding: 1.1rem; background: var(--primary); color: white; border: none; border-radius: 1.2rem; cursor: pointer; font-weight: 900; font-size: 1rem; }
    ...
    if (!window.dbCalc) {
        try {
            ...
            if (data.values) {
                ...
                // 1. CARGA DE FÁRMACOS (Columna A, desde A2) -> Para los selectores
                for (let i = 1; i < data.values.length; i++) {
                    const row = data.values[i];
                    if (row && row[0] && row[0].toString().trim().toLowerCase() !== "farmaco") {
                        window.dbCalc.push({
                ...
                // 2. MAPEO DE COLUMNAS PARA SWITCH (Fila 13 / Índice 12)
            ...
            container.innerHTML = `<div style="padding:2rem;">Error cargando datos.</div>`;
            return;
        }
    }

    // El selector lee window.dbCalc que viene de la columna A (desde A2)
    ...
    container.innerHTML = `
        <div class="calc-ui">
            ...
            <button class="btn-ejecutar" onclick="ejecutarCalculo()">CALCULAR</button>
...
window.ejecutarCalculo = function() {
    ...
    // Búsqueda en dbCalc (Columna A) para factores
    ...
    header.style.background = `hsl(${(fDest.length * 50) % 360}, 80%, 94%)`;
    header.innerHTML = `
        <div style="font-size:0.7rem; font-weight:800; opacity:0.6; text-transform:uppercase;">Dosis Objetivo</div>
        <div style="font-size:2.8rem; font-weight:900;">${Maudsley.toFixed(1)} <span style="font-size:1rem;">mg/día</span></div>
    `;

    // ESTRATEGIA: Fila (Origen en A) vs Columna (Destino en Fila 13)
    const filaIdx = o.filaIdx;
    const colIdx = window.mapSwitchCol[fDest.toUpperCase()];
    ...
    document.getElementById('res-pauta').innerHTML = `
        ...
        ${fOrig === fDest ? 'Mismo fármaco.' : window.traducirPasos(rawInstr, dosis, Maudsley)}
    `;
};

window.traducirPasos = function(rawStr, dOrig, targetMg) {
    if (!rawStr || rawStr.trim() === "" || rawStr === "NaN") return "Sin pauta específica.";
    ...
    bloques.forEach(bloque => {
        ...
        if (sujeto === 'ACTUAL') {
            ...
            if (valor.includes('TARGET')) {
                ...
                desc = `Nuevo al ${pct}% (<b>${(targetMg * pct / 100).toFixed(1)} mg</b>).`;
