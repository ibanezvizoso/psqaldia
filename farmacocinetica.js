...
<style>
    ...
    .sss-loader { text-align: center; padding: 2rem; opacity: 0.6; }
</style>
`;
...
window.fillFarmacos = function(id) {
    ...
    sel.innerHTML = `<option value="" disabled selected>Elegir...</option>` + list.map(f => `<option value="${f.farmaco}">${f.farmaco}</option>`).join('');
};
