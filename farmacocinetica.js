/**
 * Motor Farmacocinético SSS - Versión Directa
 * PSQALDÍA © 2026
 */
...
function iniciarInterfazSSS() {
    ...
    container.innerHTML = `
        <style>
            .sss-ui { padding: 1rem; }
            .sss-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem; }
            .sss-mode-selector { background: var(--primary); color: white; border: none; padding: 5px 10px; border-radius: 8px; font-weight: 700; font-size: 0.8rem; }
            .sss-card { background: var(--bg); border: 1px solid var(--border); padding: 10px; border-radius: 12px; margin-bottom: 10px; }
            ...
            .sss-ui select, .sss-ui input { width: 100%; padding: 6px; border-radius: 6px; border: 1px solid var(--border); background: var(--card); color: var(--text-main); font-size: 0.8rem; margin-bottom: 6px; box-sizing: border-box; }
            .grid-2 { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; }
            .check-row { display: flex; align-items: center; gap: 6px; font-size: 0.7rem; font-weight: 700; margin: 4px 0; color: var(--primary); }
    ...
    if (mode !== 'SWITCH') {
        container.innerHTML = `
            <div class="sss-card">
                ...
                <div class="check-row"><input type="checkbox" id="f1-ch" onchange="toggleExt()"> ¿Cambio de dosis / Taper?</div>
...
function toggleExt() {
    document.getElementById('f1-ext').style.display = document.getElementById('f1-ch').checked ? 'block' : 'none';
    renderSSS();
}
...
function renderSSS() {
    ...
    sssChart = new Chart(ctx, { type: 'line', data: { datasets: dss }, options: { responsive: true, scales: { x: { type: 'linear', title: { display: true, text: 'Días', font: {size: 10} } }, y: { min: 0, title: { display: true, text: '%', font: {size: 10} } } }, plugins: { legend: { labels: { boxWidth: 10, font: {size: 10} } } } } });
