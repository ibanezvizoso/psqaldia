...
function openAutoimmuneUI() {
    ...
    modalData.innerHTML = `
        <div class="calc-ui" style="padding: 1.2rem; display: flex; flex-direction: column; font-family: system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif;">
            <div style="position: sticky; top: 0; background: var(--card, var(--card-bg, #ffffff)); z-index: 10; padding-bottom: 0.8rem; border-bottom: 1px solid var(--border);">
...
function updatePA() {
    ...
    if (definitiva) {
        ...
        estudioBox.innerHTML = `Solicitar EEG, RM cerebral, anticuerpos en suero, LCR`;
    } else if (posible) {
        estudioBox.style.display = 'block';
...
function openRedFlags() {
    ...
    rfModal.innerHTML = `
        <div style="background: var(--card, var(--card-bg, #ffffff)); padding:1.5rem; border-radius:1.5rem; max-width:400px; width:100%; box-shadow: 0 20px 25px rgba(0,0,0,0.5); border: 1px solid var(--border); font-family: system-ui, -apple-system, sans-serif;" onclick="event.stopPropagation()">
